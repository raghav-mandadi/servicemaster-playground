...
import { users } from '../data/mockDataLoader';
    ...
    <div className="px-8 py-6">
      ...
      <div>
        ...
        <div className="flex items-center bg-[#E6E8ED] rounded-[8px] mb-2">
          {['Name', 'Role', 'Actions'].map((h, i) => (
            <div key={h} className="px-4 py-[10px] text-[16px] font-medium text-text-subtle"
              style={i === 2 ? { flex: '0 0 100px' } : { flex: '1 1 0', minWidth: 0 }}>
              {h}
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-[8px]">
          {users.map(user => (
            <div key={user.userId} className="flex items-center border border-[#ededed] rounded-[8px] bg-white hover:shadow-row-hover transition-all">
              ...
              <div className="px-4 py-4" style={{ flex: '1 1 0', minWidth: 0 }}><Tag>{ROLE_LABEL[user.role] ?? user.role}</Tag></div>
