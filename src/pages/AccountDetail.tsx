...
import { accounts, accountHealthScores } from '../data/mockDataLoader';

type Tab = 'sites' | 'buildings';
...
const frequencyColors: Record<string, string> = {
  ...
  Monthly: 'bg-[#F4F4F5] text-[#52525B] border-[#D4D4D8]',
};
...
export function AccountDetail() {
  ...
  const accountSites = accountHealthScores.filter(h => h.accountId === account.id);

  // Overall health: pick worst-tier site for the badge
  const accountHealth = accountSites.find(h => h.tier === 'red') ??
    accountSites.find(h => h.tier === 'yellow') ??
    accountSites[0] ?? null;
    ...
    <div className="px-8 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="primary"><Plus size={16} /> New Site</Button>
        </div>
      </div>
      ...
      <div className="bg-white border border-border-card rounded-[8px] p-4 mb-4 flex items-center gap-6 divide-x divide-border-card">
        {[
          ...
          { label: 'Title', value: account.primaryContact.title },
          { label: 'Phone', value: account.primaryContact.phone },
          { label: 'Email', value: account.primaryContact.email },
          { label: 'Address', value: `${account.address.street}, ${account.address.city}, ${account.address.state} ${account.address.zip}` },
      ...
      <div className="border-b border-border-card mb-4 flex gap-6">
        {(['sites', 'buildings'] as Tab[]).map(tab => (
          <button
            key={tab}
      ...
      {activeTab === 'sites' && (
        <div>
          <div className="flex justify-end mb-3">
            <Button variant="primary" size="skinny"><Plus size={14} /> New Site</Button>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-surface-header border-b border-border-header">
                  {['Site Name', 'Frequency', 'Monthly Value', 'Health Score', 'Contract End', 'Actions'].map(h => (
              ...
              <tbody>
                {accountSites.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-text-subtle">No sites yet.</td></tr>
                ) : accountSites.map(site => (
                  <tr key={site.dealId} className="border-b border-border-card hover:bg-surface-page transition-all">
                    <td className="px-4 py-4 text-[16px] font-medium text-text-primary">{site.dealName}</td>
                    <td className="px-4 py-4 text-[14px] text-text-primary capitalize">{site.serviceFrequency}</td>
                    <td className="px-4 py-4 text-[16px] font-medium text-text-primary">
                      {site.monthlyRevenue.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                    </td>
                    <td className="px-4 py-4">
                      <button onClick={() => navigate('/health')} title="View in Health module">
                        <HealthBadge score={site.score} tier={site.tier} trend={site.trend} size="sm" />
                      </button>
                    </td>
                    <td className="px-4 py-4 text-[13px] text-text-subtle">{site.contractEndDate ?? '—'}</td>
                    <td className="px-4 py-4">
        ...
        <div>
          ...
          <div className="rounded-[8px] border border-border-card overflow-hidden bg-white">
            <table className="w-full">
              <thead>
                <tr className="bg-surface-header border-b border-border-header">
                  {['Building Name', 'Sq Footage', 'Floors', 'Total Restrooms', 'Cleaning Frequency', 'Actions'].map(h => (
                    <th key={h} className="text-left px-4 py-[10px] text-[16px] font-medium text-text-subtle">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {account.buildings.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-text-subtle">No buildings yet.</td></tr>
                ) : account.buildings.map(bld => {
                  const totalRestrooms = bld.floors.reduce((sum, f) => sum + f.restroomCount, 0);
                  return (
                    <tr key={bld.buildingId} className="border-b border-border-card hover:bg-surface-page transition-all">
                      ...
                      <td className="px-4 py-4 text-[16px] text-text-primary">{bld.floors.length}</td>
                      <td className="px-4 py-4 text-[16px] text-text-primary">{totalRestrooms}</td>
                      <td className="px-4 py-4">
